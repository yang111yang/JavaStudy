...
public class ServletContextServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//使用ServletContext对象完成不同用户的数据共享
		sc.setAttribute("str", "ServletContext对象学习");
		
		//使用ServletContext对象获取项目中WebRoot目录下web.xml配置文件中的全局数据
		String name = sc.getInitParameter("name");
		System.out.println("ServletContextServlet.service()"+name);
		
		//使用ServletContext对象获取项目中WebRoot目录下资源的绝对路径
		String path = sc.getRealPath("/doc/1.txt");
		System.out.println(path);
		
		//使用ServletContext对象获取项目中WebRoot目录下资源的流对象
